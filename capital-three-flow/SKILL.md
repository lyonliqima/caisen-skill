...
## 背景资料 · 外部同类实现（观点层，非本框架结论）

📎 **[references/case-wechat-2026-09-12.md](references/case-wechat-2026-09-12.md)** —— 微信公众号「资本三流分析助手」2026-09-12 深度研判的**全文归档 + 算法逐维拆解 + 与本 skill 口径对照 + 官方源逐项数据核验**。

**引用这份资料的三条硬规则**：

1. **数据引用前先看核验表**。已查出 1 处口径错误：该文把"上半年累计 GDP 同比 4.7%"标成"Q2 同比"（官方 Q2 为 **4.3%**），致实质负利率被低估 0.4pp。**GDP 口径必须写清单季还是累计**。
2. **观点引用必须标注"外部观点"**。该文的**情景概率（50%/30%/20%）本框架不采用**，一律改译成条件式判断（"若 X 连续 N 期成立，则 Y"）。
3. **不采信不可复算的自制指数**。该文"市场风险分 73.39""行业扩张−需求差"等打分口径未公开，只能当外部参照。

**从该文学到、且本 skill 原本没有的三个公式**（已登记入 [references/methodology.md](references/methodology.md) 第九节，作为"宏观前提指标"）：

| 公式 | 作用 | 参考读数（2026-07 口径） |
|------|------|--------------------------|
| `实质负利率 = 名义利率 − CPI − (M2增速 − GDP增速)`<br>名义利率取 **Shibor 1Y** | 总纲级前提：现金购买力年缩水速度 | −2.42%（Q2 口径）／−2.02%（上半年口径） |
| `M2 − M1 剪刀差` | 资金活化度：钱是否淤在定存/企业活期 | 3.7pct（淤积）；需连续 3 周收窄才算活化 |
| `扩张−需求差 = 在建工程增速 − 营收增速`（分行业，季频） | 流量下沉到产业层：正差=过剩风险，负差=出清 | 正：综合 +33.21 / 建筑装饰 +13.51pct；负：通信 −5.91pct |

> 三者均为**纯公式登记，尚未接入 `scripts/` 流水线**；接入方案见该归档文件第六节 P1 清单。
> 另：该文流速用「年化波动率 + 市场风险分 + 估值分位」的**情绪口径**，与本 skill 的 DR007/换手率**货币口径**互补——报告可两者并列，但条件概率必须同时给无条件基准率。
...
├── references/
│   ├── methodology.md         # 费雪方程式三流量化公式全文（CFCI/CFEI/CRI/S + 第九节宏观前提指标）
│   ├── daily_framework.md     # 每日观测详细框架（指标树+综合模型+三级预警+出报机制）
│   ├── data_sources.md        # 数据源考证（akshare 函数名/状态，2026-07 核实）
│   └── case-wechat-2026-09-12.md  # 外部同类实现：公众号实战文全文归档+算法拆解+数据核验（2026-09-13）
├── scripts/
