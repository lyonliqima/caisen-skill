...
| 社会融资规模 | `macro_china_social_financing` | **master 中不存在** | 标记缺失；从 CFEI 剔除信贷周转分项并重新归一化，报告注明「CFEI 缺信贷维度」 |
...
通用规则：任何代理变量上线前必须留下一条相关性证据；任何缺失项一律「从指数中剔除并对剩余权重重新归一化」，**禁止记为 0（零）**。

| 代理关系 | 检验要求 | 未通过时的处理 |
|---|---|---|
| 北向净流入 → Δ外汇储备 | 在历史重合期（2018-01 至 2024-08）跑月频相关系数，结果写回本文件 | 相关性 <0.5 只能作方向性参考，不得进 CFCI/CRI 计算 |
| 社融 → （缺失） | 无可用源 | 从 CFEI 剔除该分项并重新归一化，报告注明「CFEI 缺信贷维度」 |
| 美元指数 → Δ外储 + 人民币汇率 | 同上跑相关性 | 同上 |

> 相关系数实证结果（由 `scripts/validate_proxy.py` 产出，首次运行后回填下表）：

| 代理关系 | Pearson | Spearman | 重合期 | 结论 |
|---|---|---|---|---|
| 北向净流入 → Δ外汇储备 | _待跑_ | _待跑_ | 2018-01 ~ 2024-08 | _待填_ |
